...
export function usePeerHost() {
  ...
  const [status, setStatus] = useState<
    "idle" | "waiting" | "connected" | "reconnecting" | "error"
  >("idle");
  ...
  const assembleFile = useCallback(
    (fileId: string) => {
      ...
      if (missingChunks.length > 0) {
        pushDebugLog(
          `[RECV] assembleFile: incomplete ${entry.chunks.size}/${entry.meta.totalChunks} chunks, missing=[${missingChunks.slice(0, 10).join(",")}${missingChunks.length > 10 ? "..." : ""}]`
        );
      ...
      for (let i = 0; i < entry.meta.totalChunks; i++) {
        ...
        if (!chunk) {
          pushDebugLog(
            `[RECV-ERR] Missing chunk ${i}/${entry.meta.totalChunks} despite check passed`
          );
    ...
    (event: MessageEvent) => {
      try {
        ...
        switch (msg.type) {
          ...
          case "file-ack": {
            // Receiver confirmed they got the complete file
    ...
    async (sendState: FileSendState) => {
      ...
      for (let i = startChunk; i < totalChunks; i++) {
        ...
        if (
          ...
          pushDebugLog(
            `[RESUME-P2P-CRASH] DataChannel crashed, will resend ${p2pSentChunks.length} chunks via relay`
          );
        }
      ...
      if (
        ...
        pushDebugLog(
          `[RESUME-RESEND] Resending ${p2pSentChunks.length} chunks via relay`
        );
        ...
        pushDebugLog(
          `[RESUME-RESEND] Done resending ${p2pSentChunks.length} chunks`
        );
      }
    ...
    (event: MessageEvent) => {
      try {
        ...
        switch (msg.type) {
          ...
          case "connected":
            pushDebugLog(
              `[WS] Client connected (reconnected=${!!msg.reconnected})`
            );
            setStatus("connected");
            setError("");
            ...
            if (
              ...
              pushDebugLog(
                "[P2P] Existing DataChannel still open, skipping re-init"
              );
            }
          ...
          case "peer-disconnected":
            if (msg.permanent) {
              ...
              pushDebugLog(
                `[WS] Peer temporarily disconnected, DC=${rtcRef.current?.dataChannel?.readyState || "none"}`
              );
              if (
  ...
  const attemptReconnect = useCallback(() => {
    ...
    reconnectTimerRef.current = setTimeout(() => {
      ...
      try {
        ...
        ws.onopen = () => {
          pushDebugLog(
            `[RECONNECT] WS reconnected, re-registering token=${tokenRef.current}`
          );
          ws.send(
            JSON.stringify({ type: "register", token: tokenRef.current })
          );
  ...
  const sendFile = useCallback(
    async (file: File, options?: SendFileOptions) => {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        // Back-pressure: use event-driven approach for P2P (onbufferedamountlow)
        // and polling for WebSocket relay
        const dc = rtcRef.current?.dataChannel;
        const dcOpen = dc?.readyState === "open";

        if (dcOpen && dc) {
          // P2P mode: wait for bufferedamountlow event if buffer is full
          // This is the WebRTC-recommended flow control approach
          const P2P_MAX_BUFFER = 512 * 1024; // 512KB — conservative to prevent DC crash
          if (dc.bufferedAmount > P2P_MAX_BUFFER) {
            await new Promise<void>(resolve => {
              const onLow = () => {
                dc.removeEventListener("bufferedamountlow", onLow);
              ...
              dc.addEventListener("bufferedamountlow", onLow);
              // Safety timeout in case event never fires (DC crashed)
              setTimeout(() => {
                dc.removeEventListener("bufferedamountlow", onLow);
                resolve();
              }, 5000);
          ...
          // Relay mode: poll WebSocket bufferedAmount
          const maxBuffer = 512 * 1024;
          let bpWaits = 0;
          while (ws && ws.bufferedAmount > maxBuffer) {
            if (abortController.signal.aborted) return;
            ...
            if (bpWaits % 500 === 0) {
              pushDebugLog(
                `[WARN] Relay BP wait ${bpWaits} at chunk ${i}, buf=${(ws.bufferedAmount / 1024).toFixed(0)}KB`
              );
            }
          }
        }
