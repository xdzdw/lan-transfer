...
import type { SendFileOptions, TransferItem } from "./usePeerHost";
import {
  createClientRTC,
  handleRTCSignaling,
  sendViaTransport,
...
import { getFolderName, type FolderTransferFile } from "@/lib/folderTransfer";
...
export function usePeerClient() {
  ...
  const folderItemIdsRef = useRef<Set<string>>(new Set());
  ...
  const sentFilesRef = useRef<
    Map<string, { file: File; totalChunks: number; sentAt: number }>
  >(new Map());
  ...
  const acquireWakeLock = useCallback(async () => {
    ...
    try {
      ...
      pushDebugLog(
        `[WAKE] Failed to acquire wake lock: ${(err as Error).message}`
      );
    }
  }, []);
  ...
  const updateItem = useCallback(
    (id: string, updates: Partial<TransferItem>) => {
      setItems(prev => {
        const next = prev.map(item =>
          item.id === id ? { ...item, ...updates } : item
        );
        const changed = next.find(item => item.id === id);
        if (!changed?.folderId) return next;
        const folder = next.find(
          item => item.id === changed.folderId && item.type === "folder"
        );
        if (!folder) return next;
        const children = next.filter(
          item => item.type === "file" && item.folderId === folder.id
        );
        const totalSize =
          folder.folderTotalSize ||
          children.reduce((sum, item) => sum + (item.size || 0), 0);
        const transferredSize = children.reduce(
          (sum, item) => sum + (item.size || 0) * ((item.progress || 0) / 100),
          0
        );
        const expectedCount = folder.folderFileCount || children.length;
        const completedCount = children.filter(
          item => item.status === "done"
        ).length;
        const status = children.some(item => item.status === "error")
          ? "error"
        ...
        const progress =
          totalSize > 0
            ? Math.min(99, Math.round((transferredSize / totalSize) * 100))
            : expectedCount > 0
              ? Math.round((completedCount / expectedCount) * 100)
              : 0;
        return next.map(item =>
          item.id === folder.id
            ? { ...item, status, progress: status === "done" ? 100 : progress }
            : item
        );
      });
    },
  ...
  const chunkRequestRetryRef = useRef<
    Map<
      string,
  ...
  const requestMissingChunks = useCallback(
    (fileId: string, missingChunks: number[]) => {
      ...
      if (retryState.attempts >= 15) {
        pushDebugLog(
          `[RETRY-ERR] Max retries (15) reached for ${fileId.slice(0, 8)}, giving up`
        );
      ...
      pushDebugLog(
        `[RETRY] Requesting ${missingChunks.length} missing chunks (attempt ${retryState.attempts}/15): [${missingChunks.slice(0, 10).join(",")}${missingChunks.length > 10 ? "..." : ""}]`
      );
      ws.send(
        JSON.stringify({
          type: "chunk-request",
      ...
      retryState.timer = setTimeout(() => {
        ...
        if (stillMissing.length > 0) {
          pushDebugLog(
            `[RETRY] Timeout — still missing ${stillMissing.length} chunks, retrying...`
          );
  ...
  const assembleFile = useCallback(
    (fileId: string) => {
      ...
      if (missingChunks.length > 0) {
        pushDebugLog(
          `[RECV] assembleFile: incomplete ${entry.received}/${entry.meta.size} bytes, ${entry.chunks.size}/${entry.meta.totalChunks} chunks, missing=[${missingChunks.slice(0, 10).join(",")}${missingChunks.length > 10 ? "..." : ""}]`
        );
      ...
      for (let i = 0; i < entry.meta.totalChunks; i++) {
        ...
        if (!chunk) {
          pushDebugLog(
            `[RECV-ERR] Missing chunk ${i}/${entry.meta.totalChunks} despite received=${entry.received}`
          );
      ...
      if (blob.size !== entry.meta.size) {
        pushDebugLog(
          `[RECV-ERR] Size mismatch: expected ${entry.meta.size}, got ${blob.size} (diff=${entry.meta.size - blob.size} bytes)`
        );
        ...
        pushDebugLog(
          `[RECV] Providing file despite size mismatch — user can still download`
        );
        updateItem(fileId, {
          progress: 100,
          status: "done",
        ...
        pushDebugLog(
          `[ACK] Sent file-ack (with size warning) for ${entry.meta.name}`
        );
        return;
      }

      pushDebugLog(
        `[RECV] DONE: ${entry.meta.name} (${(entry.meta.size / 1024 / 1024).toFixed(1)}MB, ${entry.meta.totalChunks} chunks)`
      );
  ...
  const processBinaryChunk = useCallback(
    (buffer: ArrayBuffer) => {
      ...
      if (!entry) {
        pushDebugLog(
          `[RECV-WARN] Chunk for unknown file ${fileId.slice(0, 8)}, idx=${chunkIndex}`
        );
        return;
      }
      ...
      const progress = Math.min(
        99,
        Math.round((entry.received / entry.meta.size) * 100)
      );
      ...
      if (progress >= lastLogged + 10) {
        pushDebugLog(
          `[RECV] ${progress}% | ${entry.chunks.size}/${entry.meta.totalChunks} chunks | ${(entry.received / 1024 / 1024).toFixed(1)}MB/${(entry.meta.size / 1024 / 1024).toFixed(1)}MB`
        );
        recvLogProgressRef.current.set(fileId, progress);
      }

      if (
        entry.pendingComplete &&
        entry.chunks.size >= entry.meta.totalChunks
      ) {
        assembleFile(fileId);
      }
    },
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case "file-meta": {
            ...
            if (meta.folderId && !folderItemIdsRef.current.has(meta.folderId)) {
              folderItemIdsRef.current.add(meta.folderId);
              addItem({
                id: meta.folderId,
                type: "folder",
                direction: "received",
                name: meta.folderName || "Folder",
                folderName: meta.folderName || "Folder",
                folderFileCount: meta.folderFileCount,
                folderTotalSize: meta.folderTotalSize,
                progress: 0,
                timestamp: Date.now(),
                status: "transferring",
              });
          ...
          case "file-complete": {
            ...
            if (entry) {
              ...
              pushDebugLog(
                `[RECV] file-complete signal received, have ${entry.chunks.size}/${entry.meta.totalChunks} chunks (${(entry.received / 1024 / 1024).toFixed(1)}MB/${(entry.meta.size / 1024 / 1024).toFixed(1)}MB)`
              );
              binaryQueueRef.current = binaryQueueRef.current.then(() => {
                assembleFile(msg.id);
              });
              ...
              pushDebugLog(
                `[RECV-WARN] file-complete for unknown file ${msg.id?.slice(0, 8)}`
              );
            }
            break;
          }
          ...
          case "file-ack": {
            ...
            pushDebugLog(
              `[ACK] Received file-ack for ${ackFileId?.slice(0, 8)}`
            );
            ...
            if (pendingSend && !pendingSend.completed) {
              ...
              if (ctrl) {
                ctrl.abort();
                activeSendAbortsRef.current.delete(ackFileId);
              }
  ...
  const handleRTCOffer = useCallback(
    (ws: WebSocket, offer: RTCSessionDescriptionInit) => {
      // Close any existing RTC connection before creating a new one
      if (rtcRef.current) {
        console.log(
          "[Client] Closing existing RTC before handling new offer..."
        );
      ...
      const transport = createClientRTC(
        ...
        () => {
          ...
          console.log(
            "[Client] WebRTC P2P established! Transfers will use direct connection."
          );
        },
  ...
  const resumePendingSends = useCallback(() => {
    for (const [id, sendState] of Array.from(
      pendingSendsRef.current.entries()
    )) {
      ...
      if (activeSendAbortsRef.current.has(id)) {
        pushDebugLog(
          `[RESUME-SKIP] ${sendState.file.name} already has active send loop`
        );
        continue;
      }
      pushDebugLog(
        `[RESUME] ${sendState.file.name} from chunk ${sendState.lastSentChunk + 1}/${sendState.totalChunks}`
      );
      resumeFileSend(sendState);
    }
  }, []);

  const resumeFileSend = useCallback(
    async (sendState: FileSendState) => {
      const ws = wsRef.current;
      const { id, file, totalChunks, lastSentChunk } = sendState;
      const startChunk = lastSentChunk + 1;

      pushDebugLog(
        `[RESUME] ${file.name} from chunk ${startChunk}/${totalChunks}`
      );
      ...
      const metaStr = JSON.stringify({
        ...
        meta: {
          ...
          mimeType: file.type,
          totalChunks,
          isScreenshot: sendState.isScreenshot,
          folderId: sendState.folderId,
          folderName: sendState.folderName,
          relativePath: sendState.relativePath,
          folderFileCount: sendState.folderFileCount,
          folderTotalSize: sendState.folderTotalSize,
        },
      });
      ...
      for (let i = startChunk; i < totalChunks; i++) {
        ...
        if (dcOpen && dc) {
          ...
          while (ws && ws.bufferedAmount > maxBuffer) {
            ...
            if (ws.readyState !== WebSocket.OPEN) {
              pushDebugLog(
                `[ERR] WebSocket closed during resume relay at chunk ${i}/${totalChunks}`
              );
        ...
        if (!dcStillOpen && !wsOpen) {
          pushDebugLog(
            `[ERR] Both transports dead at resume chunk ${i}/${totalChunks}`
          );
          sendState.lastSentChunk = i - 1;
          return;
        }
        ...
        if (progress >= lastLoggedProgress + 10) {
          ...
          pushDebugLog(
            `[RESUME] ${progress}% | ${elapsed.toFixed(1)}s | ${speedMBs.toFixed(2)} MB/s | mode=${actualMode} | dc=${dcState}`
          );
      ...
      setTimeout(() => {
        sentFilesRef.current.delete(id);
      }, 120_000);
    ...
    (event: MessageEvent) => {
      try {
        ...
        switch (msg.type) {
          case "connected":
            pushDebugLog(
              `[WS] Connected to host (reconnected=${!!msg.reconnected})`
            );
            setStatus("connected");
            setError("");
          ...
          case "rtc-offer":
            if (wsRef.current && msg.sdp) {
              if (
                !rtcRef.current ||
                rtcRef.current.dataChannel?.readyState !== "open"
              ) {
          ...
          case "chunk-request": {
            const { fileId, chunks: requestedChunks } = msg as {
              fileId: string;
              chunks: number[];
            };
            ...
            if (!sentFile) {
              pushDebugLog(
                `[RESEND-ERR] chunk-request for unknown/expired file ${fileId?.slice(0, 8)}`
              );
              break;
            }
            pushDebugLog(
              `[RESEND] Received chunk-request for ${requestedChunks.length} chunks: [${requestedChunks.slice(0, 10).join(",")}${requestedChunks.length > 10 ? "..." : ""}]`
            );
            ...
            (async () => {
              ...
              for (const idx of requestedChunks) {
                ...
                const combined = new Uint8Array(
                  header.byteLength + chunkData.byteLength
                );
              ...
              pushDebugLog(
                `[RESEND] Done resending ${requestedChunks.length} chunks`
              );
            })();
            break;
          }
          ...
          case "peer-disconnected":
            if (msg.permanent) {
              ...
              pushDebugLog(
                `[WS] Host temporarily disconnected, DC=${rtcRef.current?.dataChannel?.readyState || "none"}`
              );
              if (
                !rtcRef.current?.dataChannel ||
                rtcRef.current.dataChannel.readyState !== "open"
              ) {
                setTransportMode("relay");
              } else {
  ...
  const attemptReconnect = useCallback(() => {
    ...
    reconnectTimerRef.current = setTimeout(() => {
      ...
      try {
        ...
        ws.onopen = () => {
          pushDebugLog(
            `[RECONNECT] WS reconnected, rejoining token=${tokenRef.current}`
          );
          ws.send(
            JSON.stringify({
              type: "rejoin",
  ...
  const connect = useCallback(
    (inputToken: string) => {
      try {
        ...
        folderItemIdsRef.current.clear();
  ...
  const sendFile = useCallback(
    async (file: File, options?: SendFileOptions) => {
      ...
      pushDebugLog(
        `[SEND] Start: ${file.name} (${fileSizeMB}MB, ${totalChunks} chunks)`
      );
      ...
      const sendState: FileSendState = {
        ...
        folderFileCount: options?.folderFileCount,
        folderTotalSize: options?.folderTotalSize,
      };
      ...
      for (let i = 0; i < totalChunks; i++) {
        if (abortController.signal.aborted) {
          pushDebugLog(
            `[SEND] Paused at chunk ${i}/${totalChunks}, will resume`
          );
          return;
        }
        ...
        if (dcOpen && dc) {
          ...
          while (ws && ws.bufferedAmount > maxBuffer) {
            ...
            if (ws.readyState !== WebSocket.OPEN) {
              pushDebugLog(
                `[ERR] WebSocket closed during relay at chunk ${i}/${totalChunks}`
              );
              sendState.lastSentChunk = i - 1;
              return;
            }
            if (bpWaits % 500 === 0) {
              pushDebugLog(
                `[WARN] Relay back-pressure wait ${bpWaits} at chunk ${i}, buf=${(ws.bufferedAmount / 1024).toFixed(0)}KB`
              );
            }
          }
        }
        ...
        if (!dcStillOpen && !wsOpen) {
          pushDebugLog(
            `[ERR] Both transports dead at chunk ${i}/${totalChunks}`
          );
          sendState.lastSentChunk = i - 1;
          return;
        }
        ...
        if (
          ...
          pushDebugLog(
            `[P2P-CRASH] DataChannel crashed after sending ${p2pSentChunks.length} chunks via P2P. Will resend via relay after main loop.`
          );
        }
        ...
        if (progress >= lastLoggedProgress + 10) {
          ...
          const dcState = rtcRef.current?.dataChannel?.readyState || "none";
          pushDebugLog(
            `[SEND] ${progress}% | ${elapsed.toFixed(1)}s | ${speedMBs.toFixed(2)} MB/s | mode=${actualMode} | dc=${dcState}`
          );
          lastLoggedProgress = progress;
        }
      }
      ...
      if (
        dcCrashDetected &&
        p2pSentChunks.length > 0 &&
        ws?.readyState === WebSocket.OPEN
      ) {
        pushDebugLog(
          `[RESEND] Resending ${p2pSentChunks.length} chunks via relay (originally sent via P2P before crash)`
        );
        ...
        pushDebugLog(
          `[RESEND] Done resending ${p2pSentChunks.length} chunks via relay`
        );
      }
      ...
      const avgSpeed =
        file.size / (1024 * 1024) / ((Date.now() - sendStartTime) / 1000);
      pushDebugLog(
        `[SEND] DONE: ${file.name} | ${totalTime}s | avg ${avgSpeed.toFixed(2)} MB/s`
      );
    },
    [addItem, updateItem]
  );
  ...
  const sendFolder = useCallback(
    async (files: FolderTransferFile[]) => {
      const folderFiles = files.filter(
        entry => entry.file.size >= 0 && entry.relativePath
      );
      if (folderFiles.length === 0) return;
      const folderId = crypto.randomUUID();
      const folderName = getFolderName(folderFiles);
      const folderTotalSize = folderFiles.reduce(
